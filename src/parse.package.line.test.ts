import parsePackageLine, { isValidPackageLine } from './parse.package.line';
...
describe('isValidPackageLine(...)', () => {
  describe('valid', () => {
    it('works with 1 item', () => {
      assert.equal(
        isValidPackageLine('8 : (1.15,3,€34)'),
        true
      );
    });
    it('works with 2 items', () => {
      assert.equal(
        isValidPackageLine('8 : (1.15,3,€34) (9.15,3.1,€181.13)'),
        true
      );
    });
    it('works with 5 items', () => {
      assert.equal(
        isValidPackageLine('2 : (1,85.31,€29) (2,14.55,€74) (3,3.98,€16) (4,26.24,€55) (5,63.69,€52)'),
        true
      );
    });
  });
  describe('invalid', () => {
    it('fails if incorrect spacing', () => {
      assert.equal(
        isValidPackageLine('2 : (1,85.31,€29)      (2,14.55,€74)'),
        false
      );
    });
    ...
    it('fails if no max weight', () => {
      assert.equal(
        isValidPackageLine('A : (1.15,3,€34) (9.15,3.1,181.13)'),
        false
      );
    });
  });
